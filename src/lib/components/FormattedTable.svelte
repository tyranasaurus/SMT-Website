<script>
  ...
  async function merge(rowIdx, colIdx) {
    ...
    let rowCount = 1
    while (
      curColIdx + 1 < data[curRowIdx].length &&
      data[curRowIdx][curColIdx + 1] === value
    ) {
      curColIdx++
      colCount++
    }
    ...
    tableData[rowIdx].push([
      ...
      rowCount,
      colCount,
      colStyles[colIdx] ? colStyles[colIdx] : '',
    ])
  }
      ...
      <tr>
        {#if headerRow && rowIndex == 0}
          {#each row as cell, colIndex (cell)}
            <th rowspan={cell[1]} colspan={cell[2]}>
              {@html cell[0]}
            </th>
          {/each}
        {:else}
          {#each row as cell, colIndex (cell)}
            <td style={cell[3]} rowspan={cell[1]} colspan={cell[2]}>
              {@html cell[0]}
            </td>
          {/each}
        {/if}
